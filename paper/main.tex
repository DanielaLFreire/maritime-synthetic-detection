...
a structural domain gap in scale (ships occupy $\sim$39\% versus $\sim$0.1\% of the
image), object density, and capture conditions; curating the public images by
global, whole-image visual similarity does not prevent this, and an epoch
...
COCO baseline (seed-paired $p<0.05$), with gains across all object sizes and the
largest point estimate in small-object recall. An ablation
...
\caption{The domain gap between the operational and public datasets. COCO size
categories computed on normalised areas over a $640\times640$ canvas, matching
the network input resolution.}
\label{tab:gap}
\centering
\footnotesize
...
\midrule
Median vessel area (\% of image) & $0.10$ & $39$ \\
Objects per image (mean)         & $3.37$ & $1.0$ \\
...
photographs (InaTechShips pre-training set, median $39\%$), which is the core of
the domain gap.}
\label{fig:scale}
\end{figure*}
...
(1{,}348 real training images $\rightarrow$ 17{,}524 synthetic images). In the
balanced joint regime the real training set is repeated $13\times$ to match, so
one A$'$~joint epoch contains 35{,}048 images (17{,}524 real + 17{,}524
synthetic, 50/50 by construction). The train/val/test split is fixed before
generation, and no test image, annotation, or background is used in any
training stage.

For the A$'$~joint-rand ablation, real vessels are first removed by fast-marching
inpainting \cite{telea2004image} (radius 3\,px), and crops are placed by
...
is deliberately \emph{not} modelled---and the number of objects per image is
...
\caption{In-place synthetic composition. (a)~Public source image
(InaTechShips); (b)~SAM-segmented crop; (c)~operational scene (CITRA-3D-Real);
(d)~synthetic image: the crop is resized and composited at the position and
scale of each real vessel, inheriting its label.}
\label{fig:pipeline}
\end{figure*}
...
deviation on the CITRA-3D-Real test set, and significance is assessed with
two-sided seed-paired $t$-tests (seeds matched across
arms).\footnote{With $n{=}3$ seeds an exact sign-permutation test cannot attain
$p<0.25$; the paired $t$-test is therefore the appropriate small-sample choice.} We compare: \textbf{B1} (random
...
random InaTechShips $\rightarrow$ fine-tune), \textbf{A$'$ joint} (COCO $\rightarrow$
...
synthetic regimes: \textbf{A$'$ seq.} (a two-stage pipeline: 100 epochs of
pre-training on the synthetic set, then 300 epochs of fine-tuning on real
CITRA-3D-Real only---unlike A$'$~joint, the two data sources are never mixed)
and \textbf{A$'$ frozen} (as A$'$ seq., with the backbone frozen during the
synthetic stage). A volume-matched control (B2-long) oversamples the real CITRA-3D-Real images by 13$\times$ (matching the number of training images per epoch of A$'$joint) without any synthetic data, isolating the effect of training volume from synthetic diversity. Code and
...
below B2 in every seed (both $p\le0.005$), and their near-equivalence ($p=0.71$) indicates
...
and capture-context mismatch. A stricter threshold would not help: recomputing
the curation score for all 27{,}796 curated images and profiling similarity
...
($0.821\pm0.004$, below the baseline in all three seeds), and at one hundred
epochs performance approaches the random-initialisation level ($0.794\pm0.005$;
10 vs.\ 100 epochs seed-paired $p=0.028$).
...
\begin{table}[!t]
\renewcommand{\arraystretch}{1.05}
\caption{Results on the CITRA-3D-Real test set (mean $\pm$ std over 3 seeds,
in \%). $\Delta$ is the mAP50 difference versus B2 in percentage points.}
\label{tab:main}
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
...
\midrule
A$'$ joint-rand (ablation) & $84.57 \pm 0.58$ & $52.08 \pm 0.24$ & $80.4$ & $+1.06$ \\
A$'$ joint (proposed)      & $84.51 \pm 0.33$ & $52.06 \pm 0.17$ & $80.5$ & $+1.00$ \\
A$'$ frozen                & $83.42 \pm 0.39$ & $50.74 \pm 0.35$ & $77.4$ & $-0.09$ \\
B2 (COCO, reference)       & $83.51 \pm 0.20$ & $50.55 \pm 0.22$ & $78.3$ & ref \\
B2-long (volume control)   & $83.51 \pm 0.25$ & $51.00 \pm 0.38$ & $79.6$ & $+0.00$ \\
A$'$ seq.                  & $82.21 \pm 0.85$ & $49.33 \pm 0.59$ & $76.9$ & $-1.31$ \\
B1 (random init)           & $80.08 \pm 0.61$ & $47.42 \pm 0.06$ & $75.0$ & $-3.43$ \\
B (InaTech random)         & $79.45 \pm 0.46$ & $47.28 \pm 0.45$ & $73.4$ & $-4.06$ \\
A (InaTech curated)        & $79.36 \pm 0.49$ & $46.92 \pm 0.17$ & $73.5$ & $-4.15$ \\
\bottomrule
\end{tabular}
\end{table}
...
public subset. Both mAP50 and mAP50-95 degrade monotonically with exposure,
approaching the random-initialisation level (B1); the COCO baseline (B2) is the
0-epoch point. Filled markers with error bars: mean $\pm$ 1 std over 3 seeds
(0, 10, and 100 epochs); open markers: single seed (20 and 50).}
\label{fig:epochs}
\end{figure}

\subsection{In-place composition and training regime}
\label{subsec:inplace}
Adapting the public crops to the operational domain reverses the picture, but the training regime is decisive. Sequential pre-training on synthetic data (A$'$ seq.) still degrades performance ($-1.31$ on average, below the baseline in all three seeds, though not significant at $n{=}3$, $p=0.14$); extending the synthetic stage beyond 100 epochs is counter-indicated by the monotonic degradation in Fig.~\ref{fig:epochs}. Freezing the backbone during that phase is roughly neutral ($-0.09$). Only \emph{balanced joint training} (A$'$ joint), which presents real and synthetic images together in a single run, yields a gain: $+1.00$ point in mAP50, exceeding B2 in every seed ($p=0.024$; mAP50-95 $+1.50$, $p<0.001$), and Recall from $0.783$ to $0.805$ ($+2.2$ points, $p=0.013$) without loss of Precision. 
...
improves every size category in all three seeds; the improvements are significant
for medium and large objects ($p<0.05$), while the largest point estimates occur
for small objects ($\text{AR}_\text{small}$ $0.390\!\rightarrow\!0.411$, $+2.1$
points, although not significant at $n{=}3$, $p=0.13$)---relevant because
...
\begin{table}[!t]
\renewcommand{\arraystretch}{1.05}
\caption{AP and AR by object size (COCO thresholds applied in native image
pixels, $1920\times1080$; mean $\pm$ std, 3 seeds).}
\label{tab:size}
\centering
\footnotesize
...
\midrule
$\text{AP}_\text{small}$  & $0.250 \pm 0.003$ & $0.271 \pm 0.005$ \\
$\text{AP}_\text{medium}$ & $0.569 \pm 0.003$ & $0.580 \pm 0.004$ \\
$\text{AP}_\text{large}$  & $0.686 \pm 0.004$ & $0.699 \pm 0.004$ \\
$\text{AR}_\text{small}$  & $0.390 \pm 0.011$ & $0.411 \pm 0.004$ \\
$\text{AR}_\text{medium}$ & $0.649 \pm 0.002$ & $0.658 \pm 0.005$ \\
$\text{AR}_\text{large}$  & $0.756 \pm 0.003$ & $0.767 \pm 0.002$ \\
\bottomrule
\end{tabular}
\end{table}
...
alignment, and the training regime itself. Out of domain the random variant is in
...
anchoring when auditability and the absence of placement heuristics matter---every
...
generalisation is the priority.
...
shift, absolute accuracy drops and seed-to-seed variance grows, so each
synthetic arm is separated from B2 only in mean (A$'$~joint vs.\ B2, $p=0.41$),
...
the ordering---domain-adapted synthetic $>$ COCO $>$ naive public---is
preserved. A$'$~seq., which degrades in-domain, is statistically
...
about $400\times$ smaller in normalised area (Table~\ref{tab:gap},
...
are costlier than false alarms, the Recall gain at no Precision cost---with the
largest point estimate in the small-object regime (Table~\ref{tab:size})---is the
...
domain gap that global visual-similarity curation does not address and that
...
improvement in mAP50 and Recall across all object sizes, and a controlled ablation shows that
